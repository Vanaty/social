...
public class ReceiveMessageListener {
	...
	@RabbitListener(queues = RabbitConfig.CHAT_MESSAGING_QUEUE)
	public void receiveMessage(InstantChatMessage message) {
		messagingTemplate.convertAndSend("/topic/" + message.getReceiver() + "/reply", message);
	}

	public void disconnectListener(String sessionId) {
		// Handle user disconnect logic
		OnlineUser user = onlineUserRepository.findById(sessionId).orElse(null);
		if (user != null) {
